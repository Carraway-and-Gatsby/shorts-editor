...
| ADR-6 | (v2 재검토 결과) STT는 자체 호스팅 유지 | 종량 비용 없음, 오프라인 동작, 품질 격차가 자막 편집(F-22)으로 흡수 가능 | 클라우드 STT — stt.py가 프로바이더 교체 가능 구조이므로 필요 시 어댑터만 추가 |
| ADR-7 | 스토리지는 드라이버 스위치 (local/S3) | `STORAGE_DRIVER` 환경 변수로 코드 변경 없이 전환. Node·Python 양쪽 동일 규약 | 처음부터 S3 전용 — 로컬 개발/테스트 마찰 |
